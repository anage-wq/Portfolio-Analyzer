...
Designed for index fund-focused investors to evaluate true annualized performance including current portfolio value.

How to Use
Ensure Python environment has required packages installed: pandas, numpy, yfinance, scipy.

Prepare trade book CSV files and any auxiliary JSON data (e.g., stock split data).

Use the preprocessing script (portfolio_vol-dd_preprocessing.py) to clean and prepare your raw trade data.

Run the volatility and drawdown analysis script (portfolio_volatility-drawdown_analysis.py) or the Jupyter notebook (index_funds_vol-dd.ipynb) for risk metrics.

Calculate the XIRR for your portfolio or index funds using portfolio_xirr.py or xirr_for_indexes.py.

Review the outputs which include detailed per-stock metrics, portfolio aggregated volatility/drawdown, and portfolio or index fund XIRR.

Summary
This repo provides a comprehensive toolkit for Indian stock and index fund investors interested in quantitative portfolio analysis covering:

Risk assessment through volatility and drawdown.

Accurate portfolio XIRR calculations with split adjustments.

Support for symbol resolution and real-time price fetching.

Integration of multiple data preprocessing and cleaning steps for robust analytics.
